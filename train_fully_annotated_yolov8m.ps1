$ErrorActionPreference = "Stop"

$Root = Split-Path -Parent $MyInvocation.MyCommand.Path
$RunName = "yolov8m_800_e50"
$Project = Join-Path $Root "runs\fully-annotated"
$RunDir = Join-Path $Project $RunName
$ModelOut = Join-Path $Root "models\fully-annotated\yolov8m_800_e50_best.pt"
$DataYaml = "C:\Users\Vuk\Desktop\Master rad\Datasets\Master Fully Annotated.v2-annotated-valid.yolov8\data.yaml"
$TimeLog = Join-Path $RunDir "training_time.txt"

New-Item -ItemType Directory -Force -Path $RunDir | Out-Null

$sw = [System.Diagnostics.Stopwatch]::StartNew()

try {
    & (Join-Path $Root "venv\Scripts\yolo.exe") detect train `
        model=yolov8m.pt `
        data="$DataYaml" `
        epochs=50 `
        imgsz=800 `
        batch=4 `
        device=0 `
        workers=0 `
        patience=20 `
        cache=False `
        mixup=0.15 `
        rect=False `
        project="$Project" `
        name="$RunName" `
        exist_ok=True `
        plots=True

    if ($LASTEXITCODE -ne 0) {
        throw "YOLO training failed. Exit code: $LASTEXITCODE"
    }

    Copy-Item `
        -LiteralPath (Join-Path $RunDir "weights\best.pt") `
        -Destination $ModelOut `
        -Force

    "Best model copied to: $ModelOut"
}
finally {
    $sw.Stop()

    $timeText = "Training time: {0:hh\:mm\:ss}" -f $sw.Elapsed
    $timeText | Tee-Object -FilePath $TimeLog
    "Time log saved to: $TimeLog"
}
